"""
Implementação legada de banco de dados em arquivo JSON.

Mantido apenas como referência histórica. NÃO IMPORTE DESTE MÓDULO em
código de produção — o caminho oficial é PostgreSQL via
`app.core.database_postgres.PostgresUserDatabase`.
"""
import json
import logging
import os
...
DOCUMENTS_PATH = os.path.join(os.path.dirname(__file__), "../../../data/documents.json")
...
class UserDatabase:
    ...
    def _write_documents(self, data: Dict) -> None:
        try:
            with open(DOCUMENTS_PATH, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
        except Exception as e:
            logger.error(f"Erro ao escrever documentos: {e}")
            raise
    ...
    def _compact_result_payload(self, payload: Optional[Dict]) -> Optional[Dict]:
        if not isinstance(payload, dict):
            return payload
        compact = dict(payload)
        ocr_result = compact.get("ocr_result")
        if isinstance(ocr_result, dict):
            ocr_result_copy = dict(ocr_result)
            if "text" in ocr_result_copy:
                ocr_result_copy["text"] = None
            compact["ocr_result"] = ocr_result_copy
        return compact

    def _to_document(self, raw: Dict, use_compact_payload: bool = False) -> Document:
        payload = raw.get("result_payload")
        if use_compact_payload and isinstance(payload, dict):
            mapped = dict(raw)
            mapped["result_payload"] = self._compact_result_payload(payload)
            return Document(**mapped)
        return Document(**raw)

    def get_document_by_id(self, document_id: str) -> Optional[Document]:
        db = self._read_documents()
        for raw in db.get("documents", []):
            if raw.get("id") == document_id:
    ...
    def get_documents_by_clinic(self, clinic_id: str, use_compact_payload: bool = False) -> List[Document]:
        db = self._read_documents()
        items = [
            self._to_document(raw, use_compact_payload=use_compact_payload)
            for raw in db.get("documents", [])
            if raw.get("clinic_id") == clinic_id
        ]
        return sorted(items, key=lambda d: d.uploaded_at or datetime.min, reverse=True)
    ...
    def update_document(
        ...
        for idx, raw in enumerate(documents):
            ...
            if validation_status is not None:
                raw["validation_status"] = validation_status
            if exams_found is not None:
                raw["exams_found"] = exams_found
            if exams_ocr is not None:
                raw["exams_ocr"] = exams_ocr
            if exams_brnet is not None:
                raw["exams_brnet"] = exams_brnet
            if ocr_markdown is not None:
                raw["ocr_markdown"] = ocr_markdown
            if run_id is not None:
                raw["run_id"] = run_id
            if result_payload is not None:
                raw["result_payload"] = result_payload
            if reviewed_by is not None:
                raw["reviewed_by"] = reviewed_by
            if reviewed_at is not None:
                raw["reviewed_at"] = reviewed_at.isoformat()
            if approval_reason is not None:
                raw["approval_reason"] = approval_reason
            if rejection_reason is not None:
                raw["rejection_reason"] = rejection_reason
            if confidence_score is not None:
                raw["confidence_score"] = confidence_score
            if quality_score is not None:
                raw["quality_score"] = quality_score
            if mandatory_coverage is not None:
                raw["mandatory_coverage"] = mandatory_coverage
            if file_path is not None:
                raw["file_path"] = file_path
            if content_hash is not None:
                raw["content_hash"] = content_hash
            if uploaded_by_user_email is not None:
                raw["uploaded_by_user_email"] = uploaded_by_user_email
    ...
    def get_recent_document_by_hash(
        ...
        found: List[Document] = []
        for raw in db.get("documents", []):
            if raw.get("uploaded_by_user_id") != uploaded_by_user_id:
                continue
            if raw.get("content_hash") != content_hash:
                continue
            if clinic_id and raw.get("clinic_id") != clinic_id:
                continue
            try:
                uploaded_at = datetime.fromisoformat(raw.get("uploaded_at"))
                if uploaded_at < since:
                    continue
            except Exception:
                continue
            found.append(Document(**raw))
        if not found:
            return None
        return sorted(found, key=lambda d: d.uploaded_at or datetime.min, reverse=True)[0]
