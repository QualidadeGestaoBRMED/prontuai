...
export default function ValidacoesPage() {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Erro ao carregar análises de validação");
    } finally {
      setLoading(false);
    }
  }, [session?.accessToken]);
  ...
  }, [items, cpfFilter, reviewerFilter, statusFilter, textFilter]);
        ...
        <SidebarInset className="bg-sidebar group/sidebar-inset">
          <header className="flex h-16 shrink-0 items-center gap-2 px-4 md:px-6 lg:px-8 bg-sidebar text-sidebar-foreground relative before:absolute before:inset-y-3 before:-left-px before:w-px before:bg-gradient-to-b before:from-white/5 before:via-white/15 before:to-white/5 before:z-50">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-semibold">Análises de Validação</h1>
            </div>
            ...
            <div className="p-6 md:p-8 lg:p-12 space-y-6 mx-auto w-full max-w-7xl">
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">
                  Visualize apenas as análises de validação realizadas (sem depender de logs de auditoria).
                </p>
                {lastUpdatedAt && (
                ...
                {loading ? (
                  ...
                  filteredItems.map((item) => (
                    <button
                      ...
                      className="w-full text-left rounded-lg border bg-white p-4 transition hover:border-slate-300"
                    >
                      ...
                      {(item.approvalReason || item.rejectionReason) && (
          ...
          <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
            <DialogContent className="max-w-3xl">
              <DialogHeader>
                <DialogTitle>Análise de Validação</DialogTitle>
              </DialogHeader>
              {selected ? (
                <div className="space-y-4 text-sm text-slate-700">
                  ...
                  {(selected.approvalReason || selected.rejectionReason) && (
