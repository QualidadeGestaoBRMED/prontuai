...
type ChatMessageProps = {
  ...
  isTableMessage?: boolean; // Nova prop
};

export function ChatMessage({ isUser, children, isTableMessage }: ChatMessageProps) {
  return (
    <article
      className={cn(
        "flex items-start gap-4 text-[15px] leading-relaxed",
        isUser && "justify-end",
        isTableMessage && "w-full max-w-full", // Adiciona largura total para tabelas
      )}
    >
      <Image
        className={cn(
      ...
      <div
        className={cn(
          ...
          isTableMessage && "max-w-none", // Remove a largura máxima para tabelas
        )}
      >
